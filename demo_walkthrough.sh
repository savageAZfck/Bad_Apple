...
BADAPPLE=target/release/badapple
ENGINE=target/release/badapple-engine
...
env BADAPPLE_SHARD_DIR=$DEMO_DIR/r1 BADAPPLE_MESH_KEY=$MESH_KEY \
  DYLD_LIBRARY_PATH=target/release nohup "$PWD/$ENGINE" >"$DEMO_DIR/r1.log" 2>&1 &
env BADAPPLE_SHARD_DIR=$DEMO_DIR/r0 BADAPPLE_MESH_KEY=$MESH_KEY \
  DYLD_LIBRARY_PATH=target/release nohup "$PWD/$ENGINE" >"$DEMO_DIR/r0.log" 2>&1 &
...
dim  '    self-healing — the same path that will carry 671B across two studios'
echo
