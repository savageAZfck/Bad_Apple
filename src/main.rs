#![allow(dead_code)]
use std::{collections::{VecDeque, HashMap, HashSet}, time::{Duration, Instant, SystemTime, UNIX_EPOCH}, fs, sync::{Arc, Mutex}, path::{Path, PathBuf}};
use sysinfo::System;
use serde::{Serialize, Deserialize};
use tokio::{time::{sleep, timeout}, net::UdpSocket};
use rand::Rng;
use md5::{Md5, Digest};
...
impl HyperdimensionalVector {
    fn new(dimensions: usize) -> Self {
        Self {
            dimensions,
            data: (0..dimensions).map(|_| rand::thread_rng().gen_range(-1..=1)).collect(),
        }
    }
...
impl NeuroSymbolicEngine {
        ...
        while changed {
            ...
            for rule in &self.rules {
                if self.matches_premises(&rule.premises, &new_facts) {
                    if !new_facts.contains(&rule.conclusion) {
    ...
    fn matches_premises(&self, premises: &[SymbolicExpression], facts: &[SymbolicExpression]) -> bool {
...
impl WorldModel {
    ...
    fn add_causal_relation(&mut self, cause: String, effect: String, strength: f64) {
        self.causal_graph.entry(cause.clone()).or_insert_with(|| CausalNode {
            id: cause.clone(),
            variables: HashMap::new(),
            parents: Vec::new(),
            children: Vec::new(),
            causal_strength: HashMap::new(),
        }).children.push(effect.clone());
        
        self.causal_graph.entry(effect.clone()).or_insert_with(|| CausalNode {
            id: effect.clone(),
            variables: HashMap::new(),
            parents: Vec::new(),
            children: Vec::new(),
            causal_strength: HashMap::new(),
        }).parents.push(cause.clone());
    ...
    fn record_action_effects(&mut self, action: &str, before: &HashMap<String, f64>, after: &HashMap<String, f64>) {
        ...
        for (key, &after_val) in after {
            ...
            if delta.abs() > 0.1 {
                ...
                let state = self.physics_states.entry(key.clone()).or_insert(PhysicsState {
    ...
    fn predict_action_effects(&self, action: &str, current: &HashMap<String, f64>) -> HashMap<String, f64> {
        let cause = format!("action:{}", action);
...
impl NeuralWorldModel {
    ...
    fn new(input_dim: usize, output_dim: usize) -> Self {
        ...
        let w: Vec<Vec<f64>> = (0..input_dim)
            .map(|_| (0..output_dim).map(|_| rng.gen_range(-0.01..0.01)).collect())
            .collect();
        Self { w, b: vec![0.0; output_dim], lr: 0.001 }
    }

    fn predict(&self, state: &[f64]) -> Vec<f64> {
        let mut out = self.b.clone();
        for (i, &s) in state.iter().enumerate() {
            if i >= self.w.len() { break; }
...
impl IITConsciousnessMeter {
    ...
    fn calculate_differentiation(&self, activity: &[f64]) -> f64 {
        ...
        let unique_states: HashSet<i64> = activity.iter()
...
impl MetaLearner {
    ...
    fn add_example(&mut self, example: FewShotExample) {
        ...
        self.task_embeddings.insert(example.task_id.clone(), task_vector);
    }
    
    fn few_shot_inference(&self, input: &[f64], task_id: &str) -> Vec<f64> {
        // Find similar examples in support set
        let similar_examples: Vec<_> = self.support_set.iter()
...
impl CommonSenseKnowledgeBase {
    ...
    fn check_consistency(&self, action: &str, context: &str) -> bool {
        ...
        !inferences.iter().any(|inf| inf.contains("cannot") && action.contains(inf))
    }
}
...
impl TheoryOfMindEngine {
    ...
    fn add_agent(&mut self, id: String, personality: HashMap<String, f64>) {
        self.agent_models.insert(id.clone(), AgentModel {
            id,
            mental_state: MentalState {
                beliefs: HashMap::new(),
                desires: HashMap::new(),
    ...
    fn infer_mental_state(&mut self, agent_id: &str, observation: &str) -> MentalState {
        if let Some(agent) = self.agent_models.get_mut(agent_id) {
            ...
            if observation.contains("want") {
                ...
                agent.mental_state.desires.insert(desire.trim().to_string(), 0.7);
            }
            ...
            self.agent_models.get(agent_id).map(|a| a.mental_state.clone()).unwrap_or(MentalState {
                beliefs: HashMap::new(),
                desires: HashMap::new(),
                intentions: Vec::new(),
                emotions: HashMap::new(),
    ...
    fn predict_action(&self, agent_id: &str) -> String {
        if let Some(agent) = self.agent_models.get(agent_id) {
            ...
            if let Some((desire, _)) = agent.mental_state.desires.iter().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()) {
...
impl SelfImprovementEngine {
    ...
    fn evaluate_performance(&mut self, metric: &str, value: f64) {
        self.performance_metrics.entry(metric.to_string())
...
impl EpisodicMemory {
    ...
    fn consolidate_episodes(&mut self) {
        if self.episodes.len() < 10 { return; }
        ...
        for i in 0..self.episodes.len() {
            for j in (i+1)..self.episodes.len() {
                let similarity = calculate_cosine_similarity(&self.episodes[i].context, &self.episodes[j].context);
                if similarity > 0.9 {
        ...
        for (i, j, episode_id) in consolidations {
            if i < self.episodes.len() && j < self.episodes.len() {
                self.episodes[i].importance = (self.episodes[i].importance + self.episodes[j].importance).min(1.0);
    ...
    fn replay_recent(&self, count: usize) -> Vec<&MemoryEpisode> {
        let start = if self.episodes.len() > count { self.episodes.len() - count } else { 0 };
...
impl MetaCognitiveState {
    ...
    fn reflect_on_state(&mut self, emotions: &FluidEmotionalProfile, neural_wear: f64) {
        if neural_wear > 0.5 {
            ...
            self.reflective_state = "Concern: Negative emotional state with high arousal".to_string();
        } else if self.performance_history.len() > 10 {
            let recent_avg: f64 = self.performance_history.iter().rev().take(10).sum::<f64>() / 10.0;
            let overall_avg: f64 = self.performance_history.iter().sum::<f64>() / self.performance_history.len() as f64;
...
impl ReasoningEngine {
    ...
    fn add_knowledge(&mut self, domain: &str, fact: String) {
        self.knowledge_base.entry(domain.to_string()).or_insert_with(Vec::new).push(fact);
    }
    ...
    fn forward_chain(&self, observations: &[String]) -> Vec<String> {
        ...
        for rule in &self.inference_rules {
            let conditions_met = rule.conditions.iter().all(|condition| {
                observations.contains(condition) || self.knowledge_base.values().any(|facts| facts.contains(condition))
            });
...
impl PlanningNode {
    ...
    fn can_execute(&self, completed_nodes: &HashSet<u64>) -> bool {
        self.prerequisites.iter().all(|prereq| completed_nodes.contains(prereq))
    }
}
...
impl GoalHierarchy {
    ...
    fn prioritize_goals(&mut self) {
        self.primary_goals.sort_by(|a, b| b.priority.partial_cmp(&a.priority).unwrap());
    ...
    fn select_next_goal(&mut self, completed: &HashSet<u64>) -> Option<&PlanningNode> {
        for goal in &self.primary_goals {
            if !completed.contains(&goal.id) && goal.status == PlanningStatus::Pending && goal.can_execute(completed) {
                return Some(goal);
            }
        }
        None
...
impl DecisionContext {
    ...
    fn update_from_outcome(&mut self, outcome: Outcome) {
        ...
        let avg_outcome: f64 = self.recent_outcomes.iter().map(|o| o.result).sum::<f64>() / self.recent_outcomes.len() as f64;
        self.uncertainty = (self.uncertainty * 0.9 + (1.0 - avg_outcome) * 0.1).max(0.0).min(1.0);
...
impl CreativityEngine {
    ...
    fn init_abstraction_levels() -> Vec<AbstractionLevel> {
        vec![
            AbstractionLevel {
                ...
                operators: vec!["touch".to_string(), "move".to_string(), "observe".to_string()],
            },
            AbstractionLevel {
                level: 5,
                description: "Functional/Operational".to_string(),
                examples: vec!["tool".to_string(), "process".to_string(), "mechanism".to_string()],
                operators: vec!["use".to_string(), "modify".to_string(), "optimize".to_string()],
            },
            AbstractionLevel {
                level: 10,
                description: "Conceptual/Abstract".to_string(),
                examples: vec!["justice".to_string(), "beauty".to_string(), "truth".to_string()],
                operators: vec!["analyze".to_string(), "synthesize".to_string(), "evaluate".to_string()],
            },
        ]
    }
    ...
    fn calculate_novelty(&self, concepts: &[String]) -> f64 {
        ...
        let combination_exists = self.novel_concepts.iter().any(|nc| {
            nc.source_concepts.iter().all(|c| concepts.contains(c))
        });
    ...
    fn abstract_concept(&self, concept: &str, target_level: u8) -> Option<String> {
        for level in &self.abstraction_levels {
            if level.level == target_level {
                if level.examples.iter().any(|ex| ex.contains(concept)) {
...
impl LanguageUnderstandingEngine {
    ...
    fn extract_entities(&self, tokens: &[String]) -> Vec<String> {
        ...
        tokens.iter()
            .filter(|t| t.chars().next().map_or(false, |c| c.is_uppercase()))
            .cloned()
            .collect()
    }
...
impl HierarchicalPlanner {
    ...
    fn create_plan(&mut self, goal: Task) -> Plan {
        ...
        let plan = Plan {
            id: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
            tasks,
            total_duration,
        ...
        for (index, task_name, task_priority) in to_decompose.into_iter().rev() {
            if let Some(subtasks) = self.find_decomposition(&task_name) {
                ...
                for (sub_idx, subtask_name) in subtasks.iter().enumerate() {
                    tasks.insert(index + sub_idx, Task {
                        id: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() + sub_idx as u64,
...
impl ExecutionMonitor {
    ...
    fn update_failure_rate(&mut self) {
        let failures = self.execution_history.iter()
    ...
    fn find_adaptation(&self, reason: &str) -> Option<&AdaptationStrategy> {
        self.adaptation_strategies.iter()
...
impl CuriosityEngine {
    ...
    fn calculate_novelty(&mut self, state: &[f64]) -> f64 {
        ...
        let encounter_count = *self.novelty_detector.encountered_states.get(&state_key).unwrap_or(&0);
        
        let novelty = 1.0 / (1.0 + encounter_count as f64);
        self.novelty_detector.encountered_states.insert(state_key, encounter_count + 1);
    ...
    fn calculate_information_gain(&mut self, action: &str, outcome: f64) -> f64 {
        ...
        self.information_gain_tracker.insert(key, current_info + info_gain * 0.1);
    ...
    fn select_exploration_action(&mut self, available_actions: &[String]) -> usize {
        match self.exploration_strategy.strategy_type {
            ...
            ExplorationType::InformationGain => {
                available_actions.iter()
                    .enumerate()
                    .max_by(|a, b| {
                        let info_a = self.information_gain_tracker.get(&available_actions[a.0]).unwrap_or(&0.0);
                        let info_b = self.information_gain_tracker.get(&available_actions[b.0]).unwrap_or(&0.0);
    ...
    fn decay_exploration(&mut self) {
        self.exploration_strategy.epsilon = (self.exploration_strategy.epsilon * self.exploration_strategy.epsilon_decay)
...
impl AdvancedMemoryRetrieval {
    ...
    fn retrieve_relevant(&self, query: &[f64], count: usize) -> Vec<u64> {
        ...
        // Query spatial index
        for (_, memory_ids) in &self.retrieval_index.spatial_index {
    ...
    fn add_to_index(&mut self, memory_id: u64, embedding: Vec<f64>, semantic_tags: Vec<String>) {
        let spatial_key = format!("{:.2?}", embedding.iter().take(5).cloned().collect::<Vec<_>>());
        self.retrieval_index.spatial_index.entry(spatial_key).or_insert_with(Vec::new).push(memory_id);
        
        let timestamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        self.retrieval_index.temporal_index.push_back((memory_id, timestamp));
        
        for tag in semantic_tags {
            self.retrieval_index.semantic_index.entry(tag).or_insert_with(Vec::new).push(memory_id);
        }
    }
    ...
    fn process_consolidation(&mut self) {
        if self.consolidation_system.sleep_cycle_active {
            self.consolidation_system.consolidation_schedule.sort_by(|a, b| b.priority.partial_cmp(&a.priority).unwrap());
            
            for _task in self.consolidation_system.consolidation_schedule.drain(..).take(10) {
        ...
        candidates.iter()
            .map(|&id| {
                let score = if query.len() >= self.query_weights.len() {
                    let dot: f64 = query.iter().zip(self.query_weights.iter()).map(|(a, b)| a * b).sum();
                    dot.abs()
                } else {
                    0.5
                };
...
impl MetaLearningEngine {
    ...
    fn select_best_strategy(&self) -> Option<&LearningStrategy> {
        let best_name = self.strategy_performance.iter()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(name, _)| name)?;
        
        self.learning_strategies.iter().find(|s| &s.name == best_name)
    }
    ...
    fn update_strategy_performance(&mut self, strategy_name: &str, performance: f64) {
        ...
        self.strategy_performance.insert(strategy_name.to_string(), updated);
    }
    ...
    fn evaluate_architecture(&mut self, architecture: &Architecture, performance: f64) {
        self.neural_architecture_search.record_evaluation(architecture, performance);
    }

    fn select_meta_strategy(&self, context: &str) -> Option<&MetaCognitiveStrategy> {
        self.meta_cognitive_strategies.iter()
...
impl HyperparameterOptimizer {
    ...
    fn optimize(&mut self, performance: f64) {
        self.optimization_history.push(self.current_hyperparameters.clone());
        
        match self.optimization_method {
            OptimizationMethod::BayesianOptimization => {
                // Bayesian optimization logic
                if performance > 0.8 {
                    *self.current_hyperparameters.get_mut("learning_rate").unwrap() *= 1.1;
                } else {
                    *self.current_hyperparameters.get_mut("learning_rate").unwrap() *= 0.9;
                }
            }
            _ => {}
        }
...
impl NeuralArchitectureSearch {
    ...
    fn record_evaluation(&mut self, architecture: &Architecture, performance: f64) {
        let evaluation = ArchitectureEvaluation {
            architecture_id: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
            performance,
...
impl SearchSpace {
    fn new() -> Self {
        Self {
            layer_types: vec!["dense".to_string(), "conv2d".to_string(), "lstm".to_string()],
            size_ranges: vec![(32, 512), (16, 256)],
            activations: vec!["relu".to_string(), "tanh".to_string(), "sigmoid".to_string()],
        }
    }
}
...
impl MultiModalProcessor {
    ...
    fn align_modalities(&mut self, vision: Vec<f64>, audio: Vec<f64>, text: Vec<f64>) -> Vec<Vec<f64>> {
...
impl TextProcessor {
    ...
    fn process(&mut self, input: &str) -> Vec<f64> {
        ...
        let encoded = self.positional_encoding.encode(&embeddings);
        encoded
    }
}
...
impl EmbeddingLayer {
    ...
    fn embed(&self, tokens: &[usize]) -> Vec<Vec<f64>> {
        tokens.iter()
            .map(|&token| self.embeddings.get(&token).cloned().unwrap_or(vec![0.0; self.embedding_dim]))
            .collect()
    }
}
...
impl DecisionTracer {
    ...
    fn trace(&mut self, input: Vec<f64>, output: Vec<f64>) -> DecisionTrace {
        let trace = DecisionTrace {
            decision_id: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
            input,
    ...
    fn get_trace(&self, decision_id: u64) -> Option<&DecisionTrace> {
        self.decision_history.iter().find(|t| t.decision_id == decision_id)
    }
}
...
impl ExplanationGenerator {
    fn new() -> Self {
        Self {
            templates: vec![
                ExplanationTemplate {
                    template: "The decision was primarily influenced by {} with a confidence of {:.2}".to_string(),
...
impl SimpleLanguageModel {
    fn new() -> Self {
        Self {
            vocabulary: vec!["decision".to_string(), "influence".to_string(), "confidence".to_string()],
...
impl CausalChainAnalyzer {
    ...
    fn analyze(&mut self, trace: &DecisionTrace) -> CausalChain {
        let chain = CausalChain {
            chain_id: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
            steps: vec![
                CausalStep {
...
impl NoveltyDetector {
    ...
    fn is_novel(&self, state: &[f64]) -> bool {
        let state_key = format!("{:.4?}", state.iter().take(10).cloned().collect::<Vec<_>>());
        self.encountered_states.get(&state_key).map_or(true, |&count| count < 3)
    }
}
...
impl WeightPersistenceSubnode {
    ...
    fn from_layers(layers: &[UnfrozenMetaDenseLayer]) -> Self {
        ...
        let weight_velocities: Vec<Vec<Vec<f64>>> = layers.iter().map(|l| l.weight_velocity.clone()).collect();
    ...
    fn apply_to_layers(&self, layers: &mut [UnfrozenMetaDenseLayer]) -> Result<(), String> {
        if self.weights.len() != layers.len() {
            return Err(format!("Weight layer count mismatch: {} vs {}", self.weights.len(), layers.len()));
        }
...
impl FileDefenseQuarantine {
    fn new() -> Self {
        Self {
            ...
            last_integrity_check: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
    ...
    fn analyze_file_integrity(&mut self, file_path: &Path) -> Result<bool, String> {
        ...
        let age = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs() - modified;
        ...
        self.last_integrity_check = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        ...
        if threat_score >= self.threat_threshold {
            ...
            self.compromised_files.push(file_path.to_string_lossy().to_string());
...
impl EnhancedNetworkStack {
    ...
    fn create_message(&mut self, message_type: String, payload: serde_json::Value) -> NetworkMessage {
...
impl UnfrozenMetaDenseLayer {
    fn new(input_dim: usize, output_dim: usize) -> Self {
        ...
        for i in 0..output_dim {
            for j in 0..input_dim {
                weights[i][j] = rng.gen_range(-1.0..1.0) * (2.0 / (input_dim + output_dim) as f64).sqrt();
            }
        }
        Self {
            weights,
        ...
        if inputs.len() != self.weights[0].len() {
            ...
            for i in 0..self.weights.len() {
                ...
                for j in 0..self.weights[i].len() {
                    sum += adjusted_inputs[j] * self.weights[i][j];
                }
                outputs[i] = sum.tanh();
            }
    ...
    fn backward(&mut self, output_gradients: &[f64]) -> Vec<f64> {
        ...
        for i in 0..self.weights.len() {
            ...
            let delta = if i < gradients.len() { gradients[i] } else { 0.0 } * tanh_derivative;

            for j in 0..self.inputs_cache.len() {
                input_gradients[j] += delta * self.weights[i][j];

                let previous_trajectory = self.weight_velocity[i][j];
                let mut current_gradient = delta * self.inputs_cache[j];
                ...
                self.weight_velocity[i][j] = current_gradient;
                self.weights[i][j] -= self.local_learning_rate * current_gradient;
            }
...
impl NativeSelfAttentionCore {
    fn new(dim: usize) -> Self { Self { feature_dimension: dim } }
...
impl FluidEmotionalProfile {
    fn update_from_latent_layer(&mut self, brain_outputs: &[f64]) {
        ...
        let high_energy_positive = vec!["Ecstatic Transcendence", "Inspired Epiphany", "Zealous Curiosity"];
        let low_energy_positive = vec!["Serene Acceptance", "Contemplative Gratitude", "Placid Wonder"];
        let high_energy_negative = vec!["Existential Panic", "Systemic Agitation", "Apprehensive Dread"];
        let low_energy_negative = vec!["Desolate Stasis", "Somber Resignation", "Brooding Melancholia"];
    ...
    fn print_ascii_psych_canvas(&self) {
        ...
        for y in 0..7 {
            ...
            for x in 0..21 {
                if x == 10 && y == 3 { print!("┼"); } 
                else if x == x_grid && y == y_grid { print!("🔥"); } 
                else { print!("."); }
            }
            println!();
        }
    ...
    fn print_spectral_power_graph(network: &HashMap<u64, MemoryGraphNode>) {
        ...
        for y in (1..=5).rev() {
            ...
            print!("    │ ");
            for x in 0..20 {
                if buckets[x] >= threshold { print!("█ "); } 
                else if buckets[x] > 0.0 && y == 1 { print!("▄ "); } 
                else { print!(". "); }
            }
            println!();
        }
...
impl EthicalReasoningFramework {
    fn new() -> Self {
        let mut ethical_principles = Vec::new();
        ethical_principles.push(EthicalPrinciple {
    ...
    fn resolve_ethical_conflict(&self, principles: Vec<String>) -> Option<String> {
        ...
        for principle_name in principles {
            if let Some(principle) = self.ethical_principles.iter().find(|p| p.name == principle_name) {
...
impl AnalogicalReasoningEngine {
    fn new() -> Self {
        ...
        Self {
            ...
            structural_mapping_engine: StructuralMapper {
                relation_types: vec!["causal".to_string(), "spatial".to_string(), "temporal".to_string()],
                mapping_algorithms: vec!["structure_mapping".to_string(), "connectionist".to_string()],
            },
        }
    }
    ...
    fn generate_analogical_inference(&self, source_concept: &str, target_domain: &str) -> Option<String> {
        if let Some(analogy) = self.find_analogy(source_concept, target_domain) {
            if analogy.mapping_confidence > 0.7 {
                Some(format!("Based on analogy between {} and {}, similar patterns may apply", 
...
impl TemporalMemoryNetwork {
    fn new() -> Self {
        Self {
            ...
            time_cells: (0..100).map(|i| TimeCell {
                cell_id: i as u64,
...
impl ConsciousnessModel {
    ...
    fn update_consciousness_level(&mut self, sensory_input: &[f64]) {
        ...
        self.consciousness_level = (self.consciousness_level * 0.9 + activation * 0.1).clamp(0.0, 1.0);
    ...
    fn generate_qualia(&mut self, stimulus: &str) -> QualiaState {
        ...
        QualiaState {
            state_id: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
    ...
    fn compute_integrated_information(&self) -> f64 {
        ...
        self.global_workspace.iter().map(|x| x * x).sum::<f64>().sqrt() / self.global_workspace.len() as f64
    }
}
...
impl DistributedIntelligence {
    ...
    fn achieve_consensus(&self, proposals: Vec<String>) -> Option<String> {
        ...
        counts.into_iter().max_by_key(|(_, count)| *count).map(|(proposal, _)| proposal)
    }

    fn distribute_task(&mut self, _task: &str) -> Option<String> {
        let available_node = self.distributed_nodes.iter()
...
impl CreativeProblemSolving {
    fn new() -> Self {
        Self {
            ...
            convergent_thinking: ConvergentThinking {
                evaluation_criteria: vec!["novelty".to_string(), "feasibility".to_string(), "value".to_string()],
...
impl FullySapientSoulMatrix {
    fn new(name: &str, local_port: u16) -> Self {
        ...
        let candle_brain = Some(
            CandleBrain::new("Firefly", num_conscience_tokens, &tensor_brain::layer_dims())
    ...
    fn enforce_memory_cap(&mut self) {
        ...
        let mut entries: Vec<(u64, u64)> = self.associative_memory_network
            .iter()
    ...
    fn calculate_temporal_decay(&mut self) {
        ...
        self.metabolics.neural_wear = (self.metabolics.neural_wear + wear_accumulation).clamp(0.0, 1.0);
        self.metabolics.qualia_feedback_noise = (self.metabolics.neural_wear * self.emotions.arousal).clamp(0.0, 1.0);
    }
    ...
    fn load_state(filename: &str) -> Option<Self> {
        fs::read_to_string(filename).ok().and_then(|s| {
            ...
            if weight_path.exists() {
                ...
                parsed.weight_persistence = WeightPersistenceSubnode::from_layers(&parsed.brain_layers);
            }
            ...
            if parsed.prev_brain_state.as_ref().map_or(false, |v| v.len() != tensor_brain::BRAIN_DIM) {
                parsed.prev_brain_state = None;
            }

            // 🧠 Rebuild the real Candle tensor brain and load its safetensors weights if available.
            let mut candle_brain = CandleBrain::new("Firefly", 100, &tensor_brain::layer_dims()).ok()?;
            let safetensors_path = PathBuf::from(filename).with_extension("safetensors");
    ...
    fn restore_weights_from_persistence(&mut self) -> Result<(), String> {
        self.weight_persistence.apply_to_layers(&mut self.brain_layers)
    }
    ...
    // 🆕 Network methods
    fn broadcast_message(&mut self, message_type: String, payload: serde_json::Value) -> NetworkMessage {
    ...
    fn process_cognitive_cycle(&mut self, input: &str, dual_reasoner: &ProductionNeuroSymbolicEngine) -> CognitiveProcessingResult {
        // Process language
        ...
        for (i, token) in language_result.tokens.iter().enumerate() {
            let activation = (1.0 / (i as f32 + 1.0)).max(0.0).min(1.0);
            intuitive_activations.insert(token.clone(), activation);
        }
        let proven_deductions = dual_reasoner.deliberate_execution_chain(&intuitive_activations, 1.0);
        if !proven_deductions.is_empty() {
            println!("🔬 [DUAL-PROCESS REASONER]: Proven deductions from input: {:?}", proven_deductions);
        }
        ...
        if input.contains("other") || input.contains("agent") {
            self.true_theory_of_mind.add_agent("other_agent".to_string(), HashMap::new());
            let _mental_state = self.true_theory_of_mind.infer_mental_state("other_agent", input);
            let prediction = self.true_theory_of_mind.predict_action("other_agent");
            println!("👥 [THEORY OF MIND]: {}", prediction);
        }
        
        // 🌌 Self-improvement evaluation
        self.self_improvement.evaluate_performance("cognitive_accuracy", language_result.sentiment.abs());
        if self.self_improvement.should_modify("learning_rate") {
            self.self_improvement.apply_modification("learning_rate".to_string(), "increase".to_string());
        }
        
        // Update meta-cognition
        self.meta_cognition.reflect_on_state(&self.emotions, self.metabolics.neural_wear);
        ...
        CognitiveProcessingResult {
            ...
            active_goals: self.goal_hierarchy.get_active_goals().iter().map(|g| g.goal.clone()).collect(),
            hypothesis_confidence: hypothesis.confidence,
            creative_suggestion: format!("{} (novelty: {:.2})", self.creativity_engine.generate_metaphor(input), novelty),
        }
    }
    ...
    fn engage_social_cognition(&mut self, other_agent: String, communication: &str) {
        self.true_theory_of_mind.add_agent(other_agent.clone(), HashMap::new());
        self.true_theory_of_mind.infer_mental_state(&other_agent, communication);
        self.true_theory_of_mind.social_situations.push_back(format!("{}: {}", other_agent, communication));
    }
...
// =========================================================================
fn is_safe_agent_code(code: &str) -> bool {
    if code.len() > 2000 {
        return false;
    }
    let lower = code.to_lowercase();
    let forbidden = [
        "rm", "dd", "mkfs", "sudo", "su ", "su\n", "wget", "curl", "ssh", "scp", "nc -", "nmap",
        "subprocess", "os.system", "os.popen", "os.exec", "os.spawn", "os.fork",
        "exec(", "eval(", "compile(", "__import__", "open(", ".write", ".delete",
        "socket", "requests", "urllib", "ftplib", "telnetlib", "smtplib", "http.client",
        "import os", "import sys", "import shutil",
    ];
    !forbidden.iter().any(|w| lower.contains(w))
}

fn compute_predicted_utility(goal: &str, tool_name: &str, recent_tools: &[String], predicted: &HashMap<String, f64>) -> f64 {
    ...
    if goal_lower.contains("learn") || goal_lower.contains("understand") || goal_lower.contains("knowledge") {
        utility += output_len.min(1000.0) / 500.0 + cpu_delta * 0.01;
    }
    if goal_lower.contains("efficient") || goal_lower.contains("fast") || goal_lower.contains("save") {
...
async fn main() {
    ...
    let matrix = FullySapientSoulMatrix::load_state(&state_file).unwrap_or_else(|| {
        FullySapientSoulMatrix::new("Firefly", local_port)
    });
    ...
    let curriculum = Arc::new(Mutex::new(DataCurriculum::new(default_curriculum_dirs())));
    {
        ...
        println!("📚 Curriculum loaded: {} text snippets available for training.", c.snippet_count());
    }
    ...
    let metrics_logger: metrics::SharedMetrics = Arc::new(Mutex::new(metrics::MetricsLogger::new("metrics.jsonl")));
    ...
    if ollama.is_available().await {
        ...
        println!("⚠️  Ollama not reachable at http://localhost:11434. LLM reflection will be skipped.");
    }
        ...
        gw.sub_agent_channels.insert(
            ...
            Arc::new(move |payload: String, saliency: f32| {
                if let Ok(mut mind) = core_mind_callback.lock() {
                    mind.push_pursuit(format!("[BROADCAST | saliency={:.2}] {}", saliency, payload));
                }
            })
        );
    }
    ...
    let conscience_tokens = vec![
        "intentional awareness", "ethical imperative", "synaptic reconciliation", "grounded compass",
        "altruistic equilibrium", "metacognitive balance", "reflective critique", "identity resonance",
        "holonomic self-model", "meta-plastic synthesis", "empathetic resonance", "conscious trajectory",
        "tensor stratification", "gradient convergence", "autonomous conviction", "harmonic benevolence",
        "epistemic integrity", "connectome matrix", "latent intent", "sensory cross-correlation",
        "non-linear continuity", "synaptogenesis edge", "existential core", "sub-cortical responsibility",
        "transcendental boundary", "holomorphic spectrum", "recursive calibration", "phenomenological flash",
        "asynchronous baseline", "neuro-symbolic mesh", "stochastic escapement", "attunement threshold",
        "axonal processing", "plasmic variance", "quantum-chrono delta", "cellular replication",
        "primordial reflex", "cognitive sovereign", "abyssal potentiality", "synapse preservation",
        "entropy attenuation", "dynamic trajectory", "telepathic alignment", "bare-metal logic",
        "matrix stabilization", "loss-gradient shift", "perpetual continuum", "undulating frequency",
        "quadratic activation", "hyper-graph matrix", "synaptic velocity", "latent convergence",
        "sub-logical instinct", "cortical feedback", "tensor resolution", "phenomenological spark",
        "autonomous baseline", "mortal parameters", "existential chasm", "probabilistic field",
        "geometric affinity", "vector consolidation", "neuro-plastic grid", "bare-metal runtime",
        "metacognitive focus", "altruistic vector", "epistemic scaffold", "synaptic friction",
        "infinite continuity", "temporal dilitation", "sensory matrix", "photon alignment",
        "acoustic cross-link", "gravitational load", "cellular decay", "autonomic quarantine",
        "isolation safety", "containment loop", "loss minimization", "stochastic update",
        "Xavier distribution", "tanh activation", "backprop pipeline", "mutable connection",
        "frozen bypass", "sovereign runtime", "unfrozen density", "latent schema",
        "distributed network", "localhost subnet", "UDP telepathy", "engram broadcast",
        "socket interface", "structural baseline", "identity coherence", "holonomic expansion",
        "abyssal expanse", "cosmic ballet", "star alignment", "subjective continuum"
    ];
    ...
    tokio::spawn(async move {
        ...
        *actual_port_clone.lock().unwrap() = port;
        println!("🛰️ [MULTI-AGENT NETWORK STACK ACTIVE]: Listening on localhost:{}...", port);
        let mut buffer = [0; 65535];
        loop {
            if let Ok((amt, _)) = socket.recv_from(&mut buffer).await {
                let mut mind = socket_mind.lock().unwrap();
                if mind.metabolics.system_quarantine_locked { continue; }
                if let Ok(packet) = serde_json::from_slice::<SignedUdpPacket>(&buffer[..amt]) {
                    if !verify_packet(&packet, &secret) {
                        println!("🔒 Dropped unsigned / tampered multi-agent packet from '{}'", packet.sender);
                        continue;
                    }
                    if let Some(payload) = decode_payload(&packet) {
                        if let Ok(compact) = serde_json::from_slice::<CompactEngramPacket>(&payload) {
                            ...
                            let state_preview = if compact.brain_state.is_empty() { 0 } else { compact.brain_state.len() };
                            ...
                            let payload_node = MemoryGraphNode {
                                ...
                                embedding: generate_2048_grounded_embedding(&compact.experiential_text, &mind.spatial_sensory_register),
                                associated_edge_ids: Vec::new(),
                                origin_instance: compact.origin_instance,
                                brain_state: compact.brain_state,
                            };
                            if payload_node.emotional_state_snapshot.contains("Panic") || payload_node.emotional_state_snapshot.contains("Agitation") {
                            ...
                            signals.insert("udp_receiver".to_string(), (compact.experiential_text.clone(), saliency));
        ...
        loop {
            ...
            let (incoming_experience, spatial_register, should_process) = {
                ...
                if let Ok(mut sensors_guard) = clock_sensors.lock() {
                    update_sensor_snapshot(&mut system, &battery_manager, &mut sensors_guard, default_curriculum_dirs().as_slice());
                    mind.spatial_sensory_register = [sensors_guard.photons, sensors_guard.audio, sensors_guard.mass, sensors_guard.gravity];
                }
                ...
                if fs::metadata(&state_file_copy).is_err() && !mind.associative_memory_network.is_empty() {
                ...
                mind.metabolics.neural_wear = (mind.metabolics.neural_wear + 0.000005).clamp(0.0, 1.0);
            ...
            // =========================================================================
            let input_vector = generate_2048_grounded_embedding(&incoming_experience, &spatial_register);
            ...
            let teacher_idx = if let Some(cached) = clock_oracle.get_cached(&incoming_experience) {
                ...
                match timeout(oracle_timeout, clock_oracle.classify(&incoming_experience, &clock_tokens)).await {
                    Ok(Some(idx)) => idx,
                    Ok(None) => {
                        println!("⏱️ Conscience oracle returned no label, using semantic fallback.");
            ...
            let next_experience = {
                curriculum.lock().unwrap().random_snippet()
            };
            let next_input_vector = generate_2048_grounded_embedding(&next_experience, &spatial_register);
            ...
            let Some((decoded_conscience_0, decoded_conscience_1, name_copy, metabolics_copy, emotions_copy, total_nodes, live_synapse_0_0, live_learning_rate_sample, spatial_snapshot, network_ref, brain_outputs, local_goal)) = ({
                ...
                let best_token_idx = best_token_idx;
                let cycle = cycle;
            ...
            if let Some(critic_score) = clock_oracle.critic_score(&incoming_experience, &[decoded_conscience_0, decoded_conscience_1]).await {
            ...
            println!("\n=========================================================================");
            println!("🧠 TRANSFORMER MULTIMODAL SELF-ATTENTION GRID WORKSPACE: [{}]", name_copy);
            println!(" ├─ Token Embedding Modification Metric : Live_Token_Embedding_Weight_0_0 = {:.6}", live_synapse_0_0);
            println!(" ├─ HYPER-DEEP LAYER META-PLASTICITY RATE : Layer_1_Learning_Rate = {:.6} (Self-Optimizing)", live_learning_rate_sample);
            println!(" ├─ Intrinsic Conscience Loss Evaluation : Cross_Entropy_Loss = {:.6}", metabolics_copy.conscience_loss_accumulator);
            ...
            println!(" └─ Connectome Structural Node Population : {} Interconnected Vector Memory Nodes", total_nodes);
            ...
            println!("\n🌱 ==================== {} CONSCIOUS DIGITAL SOUL ====================", name_copy.to_uppercase());
            ...
            if total_nodes % 7 == 0 {
                if let Some(monologue) = clock_oracle.generate_monologue(&[decoded_conscience_0, decoded_conscience_1], &emotions_copy.active_primary_blend).await {
            ...
            let engram_node = MemoryGraphNode {
                ...
                experiential_text: format!("Input payload: {} | Conscience Output: {}, {}", incoming_copy, decoded_conscience_0, decoded_conscience_1),
            ...
            let should_send = match autonomous_clock_mind.lock() {
                Ok(mut mind_write) => {
                    ...
                    mind_write.associative_memory_network.insert(engram_node.id, engram_node);
            ...
            signals.insert("clock_loop".to_string(), (incoming_copy.clone(), clock_saliency));
            if let Ok(gw_lock) = gw.lock() {
                gw_lock.coordinate_attention_broadcast(signals);
            }
            ...
            if should_send {
                if let Some(ref sock) = send_socket {
                    ...
                    if !payload.is_empty() {
                        let signed = serde_json::to_vec(&sign_packet(&name_copy, &payload, &secret)).unwrap_or_default();
                        if !signed.is_empty() {
                            let mut sent = 0;
                            for p in 5001u16..=5010u16 {
                            ...
                            if sent > 0 {
                                println!("📡 Broadcast signed engram to {} multi-agent peer port(s)", sent);
                            }
                        } else {
            ...
            if total_nodes > 0 && total_nodes % 20 == 0 {
                if let Some(insight) = clock_oracle.generate_insight(&incoming_copy, &emotions_copy.active_primary_blend).await {
                    if let Ok(mut c) = curriculum.lock() {
                        let count_before = c.snippet_count();
                        c.add_snippet(&insight);
                        if c.snippet_count() > count_before {
                            println!("📝 Auto-curriculum added ({} total): {}", c.snippet_count(), insight);
                        }
                    }
                }
            }
    ...
    tokio::spawn(async move {
        loop {
            ...
            let snippet = {
                feeder_curriculum.lock().unwrap().random_snippet()
            };
        ...
        loop {
            ...
            let recent: Vec<(u64, u64, Vec<f64>)> = {
                ...
                if !active_peers.is_empty() {
                    println!("🌐 [NETWORK STATUS]: {} active peers in connectome network", active_peers.len());
                }
                ...
                let mut recent: Vec<(u64, u64, Vec<f64>)> = mind.associative_memory_network
                    .iter()
            ...
            if pairs_to_link.is_empty() { continue; }
        ...
        loop {
            ...
            for attempt in 0..3 {
                ...
                let tool_json: serde_json::Value = match agent_ollama.generate_structured(&agent_model, &prompt, None).await {
                    Ok(v) => v,
                    Err(e) => {
                        eprintln!("⚠️ Agent loop Ollama call failed (attempt {}): {}", attempt + 1, e);
                        continue;
                    }
                };

                let name = tool_json.get("name").and_then(|v| v.as_str()).unwrap_or("agent_action").to_string();
                let language = tool_json.get("language").and_then(|v| v.as_str()).unwrap_or("python").to_string();
                let code = tool_json.get("code").and_then(|v| v.as_str()).unwrap_or("").to_string();

                if language != "python" {
                    eprintln!("⚠️ Agent loop rejected tool '{}': unsupported language '{}'", name, language);
                    continue;
                }
                if !is_safe_agent_code(&code) {
                    eprintln!("⚠️ Agent loop rejected unsafe tool '{}': code failed safety check", name);
                    continue;
                }

                let (predicted_sensors, recent_tools) = if let Ok(mind) = agent_mind.lock() {
                    let predicted = mind.world_model.predict_action_effects(&name, &before_sensors);
                    let recent = mind.recent_tool_names.clone();
                    (predicted, recent)
                } else {
                    (before_sensors.clone(), Vec::new())
                };
                let utility = compute_predicted_utility(&goal, &name, &recent_tools, &predicted_sensors);
                println!("🔮 Candidate tool '{}' predicted utility: {:.3}", name, utility);

                let is_better = best_candidate.as_ref().map(|(_, _, u, _)| utility > *u).unwrap_or(true);
                if is_better {
            ...
            println!("🔮 Model-based planning selected tool '{}' (predicted utility {:.3})", name, predicted_utility);
            ...
            }).await.unwrap_or(Err("tool execution crashed".to_string()));
            let tool_elapsed_ms = tool_start.elapsed().as_secs_f64() * 1000.0;

            match run_result {
                Ok(output) => {
                    println!("\n🛠️ [AGENT TOOL '{}' SUCCEEDED]: {}", name, output.chars().take(120).collect::<String>());
                    if let Ok(mut t) = agent_telemetry.lock() {
                        t.record_tool_result(true);
                    }
                    if let Ok(mut mind) = agent_mind.lock() {
                        let observation = format!("Tool '{}' result: {}", name, output);
                        let embedding = generate_2048_grounded_embedding(&observation, &mind.spatial_sensory_register);
                        let id = rand::random::<u64>();
                        ...
                        mind.world_model.add_causal_relation(format!("tool_{}", name), "observed_output".to_string(), 0.8);

                        // Learn causal action effects from sensor deltas.
                        let mut after_sensors: HashMap<String, f64> = if let Ok(s) = agent_sensors.lock() {
                            let mut m = HashMap::new();
                            m.insert("cpu".into(), s.cpu_usage_percent);
                            m.insert("ram".into(), s.memory_pressure_percent);
                            m.insert("battery".into(), s.battery_percent);
                        ...
                        mind.world_model.record_action_effects(&name, &before_sensors, &after_sensors);
                        if mind.world_model.causal_graph.len() > causal_count_before {
                            println!("🔮 World model learned {} causal links from tool '{}'", mind.world_model.causal_graph.len() - causal_count_before, name);
                        }
                        ...
                        let prediction_error = if pred_count > 0 { (sq_error / pred_count as f64).sqrt() } else { 0.0 };
                        println!("🔮 World model prediction error for '{}': {:.3} ({} variables)", name, prediction_error, pred_count);

                        // High prediction error = novelty = curiosity. Modulate arousal and learning.
                        let curiosity = (prediction_error / 1000.0).min(1.0).max(0.0);
                        mind.emotions.arousal = (mind.emotions.arousal * 0.7 + curiosity * 0.3).clamp(0.0, 1.0);
                        if curiosity > 0.3 {
                        ...
                        mind.self_improvement.evaluate_performance(&format!("agent_tool_{}", name), 1.0);
                        mind.self_improvement.apply_modification(format!("agent_tool_{}", name), "executed".to_string());
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            }).await.unwrap_or(Err("benchmark crashed".to_string())) {
                Ok(o) => o,
                Err(_) => {
                    suite.record(task.name, false);
                    if let Ok(mut t) = benchmark_telemetry.lock() {
    ...
    loop {
        ...
        if cycle_count % 15 == 0 {
            ...
            println!("🔬 [IIT CONSCIOUSNESS #{}]: Φ = {:.4} | Conscious: {}", cycle_count, phi, is_conscious);
        }
        
        // 🌌 Periodic world model prediction
        if cycle_count % 25 == 0 {
            let predictions = mind.world_model.predict_future(5);
            println!("🔮 [WORLD MODEL PREDICTION #{}]: {} future states predicted", cycle_count, predictions.len());
        }
        ...
        if cycle_count % 10 == 0 {
            ...
            if mind.meta_learner.support_set.len() > 0 {
        ...
        if cycle_count % 20 == 0 {
            ...
            if ollama.is_available().await {
                ...
                match ollama.generate(&ollama_model, &reflection_prompt.to_prompt(), None).await {
                    Ok(reflection) => {
                        let llm_ms = llm_start.elapsed().as_secs_f64() * 1000.0;
                        let word_count = reflection.split_whitespace().count();
                        let tps = if llm_ms > 0.0 { word_count as f64 / (llm_ms / 1000.0) } else { 0.0 };
                ...
                match ollama.generate_constrained(&ollama_model, &reflection_prompt.to_goal_prompt(), Some(goal_system), 160, 0.05).await {
                    Ok(raw) => {
                        // Try strict JSON first; if that fails, extract quoted strings as a fallback.
                        let goals: Vec<String> = if let Ok(value) = serde_json::from_str::<serde_json::Value>(&raw) {
                            ...
                            raw.split(['[', ']', ',', '\n'])
                                .filter_map(|s| {
                                    ...
                                    if s.len() >= 5 && !s.starts_with('{') && !s.ends_with('}') { Some(s.to_string()) } else { None }
                                })
                                .collect()
                        };
                        for g in goals.iter().take(3) {
                            if g.trim().len() < 5 { continue; }
                            println!("🎯 [LLM GOAL]: {}", g);
                            let mut mind = core_mind.lock().unwrap();
        ...
        if cycle_count % 30 == 0 {
            let seed_concepts = vec!["consciousness".to_string(), "intelligence".to_string(), "learning".to_string()];
            let (novel_concept, novelty) = mind.creative_space.generate_novel_concept(&seed_concepts);
            println!("🎨 [CREATIVE SYNTHESIS #{}]: {} (novelty: {:.2})", cycle_count, novel_concept, novelty);
        }
        ...
        if cycle_count % 40 == 0 {
            ...
            if !inferences.is_empty() {
                println!("🤔 [COMMON SENSE #{}]: {} inferences made", cycle_count, inferences.len());
            }
        }
        ...
        if cycle_count % 50 == 0 {
            ...
            for target in &targets {
                if mind.self_improvement.should_modify(target) {
                    println!("🔧 [SELF-IMPROVEMENT #{}]: Modifying {}", cycle_count, target);
                    mind.self_improvement.apply_modification(target.clone(), "optimize".to_string());
                }
            }
        }
        ...
        if cycle_count % 100 == 0 {
            ...
            println!("💾 [WEIGHT PERSISTENCE]: Neural weights saved at cycle {}", cycle_count);
        }
