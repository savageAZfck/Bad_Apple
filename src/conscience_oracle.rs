...
use crate::{calculate_cosine_similarity, generate_2048_grounded_embedding};
...
impl ConscienceOracle {
    ...
    pub async fn classify(&mut self, text: &str, tokens: &[&str]) -> Option<usize> {
        ...
        let response = self
            .client
            .generate_constrained(&self.model, &prompt, Some(system), 24, 0.2)
            .await
            .ok()?;
        ...
        for (idx, token) in tokens.iter().enumerate() {
            ...
            let token_words: std::collections::HashSet<&str> =
                token_low.split_whitespace().collect();
    ...
    pub async fn critic_score(&self, input: &str, top_tokens: &[&str]) -> Option<f64> {
        ...
        let resp = self
            .client
            .generate_constrained(&self.model, &prompt, Some(system), 8, 0.05)
            .await
            .ok()?;
        let score = resp
            .split_whitespace()
    ...
    pub async fn generate_monologue(
        ...
        let resp = self
            .client
            .generate_constrained(&self.model, &prompt, Some(system), 40, 0.7)
            .await
            .ok()?;
        let text = resp
            .trim()
    ...
    pub async fn generate_insight(
        ...
        let resp = self
            .client
            .generate_constrained(&self.model, &prompt, Some(system), 48, 0.7)
            .await
            .ok()?;
        let insight = resp
            .trim()
