...
_AMBIENT_PATTERNS = [
    ...
    (
        re.compile(r"^(.+?)\s*[-—]\s*(.+?)\s*[-—]\s*(Code|Xcode|Nova|Fleet|Cursor|Zed|IntelliJ).*", re.I),
    ...
    (
        re.compile(r"^(.+?)\s*[-—]\s*(GitHub|Google Search|Gmail|YouTube|Notion|Linear|Figma|Slack|Discord|X|Twitter).*", re.I),
        ["User has {g1} open for {g0}.", "User is looking at {g0} on {g1}."],
    ),
    # Mail: "Inbox (1) - foo@gmail.com - Mail"
    (
        re.compile(r"^(Inbox|Drafts|Sent).*?[-—]\s*(.+?)\s*[-—]\s*Mail", re.I),
        ["User is using Mail for {g1}."],
    ),
    # Xcode default: "Bad Apple — main.swift"
    (
        re.compile(r"^(.+?)\s*[-—]\s*(.+?\.swift|.+?\.m|.+?\.mm|.+?\.py|.+?\.rs|.+?\.js)$", re.I),
